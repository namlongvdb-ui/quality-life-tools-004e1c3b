...
CREATE POLICY "Admin can update roles" ON public.user_roles FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view signatures" ON public.digital_signatures FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admin can insert signatures" ON public.digital_signatures FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admin can update signatures" ON public.digital_signatures FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can update own signature" ON public.digital_signatures FOR UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Service role can delete signatures" ON public.digital_signatures FOR DELETE TO service_role USING (true);

-- Voucher signatures policies
CREATE POLICY "Users can view voucher signatures" ON public.voucher_signatures FOR SELECT TO authenticated USING (true);
CREATE POLICY "Signers can create voucher signatures" ON public.voucher_signatures FOR INSERT TO authenticated WITH CHECK (auth.uid() = signer_id);

-- Pending vouchers policies
CREATE POLICY "Authenticated can view pending vouchers" ON public.pending_vouchers FOR SELECT TO authenticated USING (auth.uid() IS NOT NULL);
CREATE POLICY "Creator can insert pending vouchers" ON public.pending_vouchers FOR INSERT TO authenticated WITH CHECK (auth.uid()::text = created_by);
CREATE POLICY "Signer roles can update pending vouchers" ON public.pending_vouchers FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'lanh_dao') OR public.has_role(auth.uid(), 'ke_toan') OR public.has_role(auth.uid(), 'phu_trach_dia_ban') OR public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admin or creator can delete pending vouchers" ON public.pending_vouchers FOR DELETE TO authenticated USING (public.has_role(auth.uid(), 'admin') OR auth.uid()::text = created_by);
...
BEGIN
  IF OLD.role = 'admin' THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Cannot delete admin role from default admin account';
    END IF;
  END IF;
  RETURN OLD;
END;
